...
const Dashboard = () => {
  ...
  function dedupeByApplicationId(applicants) {
    const dedupedMap = new Map();

    for (const applicant of applicants) {
      const existing = dedupedMap.get(applicant.applicationid);
      if (!existing || applicant.ID > existing.ID) {
        dedupedMap.set(applicant.applicationid, applicant);
      }
    }
  ...
  // ✅ Main fetch function (outside useEffect)
  const fetchData = async () => {
    setLoading(true);

    try {
      const response = await fetch(`${SERVER_URL}/api/applicantList`);
      const resData = await response.json();
      const rawData = resData.data || [];

      // Deduplicate
      const dedupedData = dedupeByApplicationId(rawData);

      // Set rows
      setRows(dedupedData);

      // Set filter options based on deduped data
      setFilterOptions({
        gender: [...new Set(dedupedData.map((d) => d.gender).filter(Boolean))],
        candidatesource: [
          ...new Set(dedupedData.map((d) => d.candidatesource).filter(Boolean)),
        ],
        candidatetype: [
          ...new Set(dedupedData.map((d) => d.candidatetype).filter(Boolean)),
        ],
        applied_role: [
          ...new Set(dedupedData.map((d) => d.applied_role).filter(Boolean)),
        ],
        profiled_account: [
          ...new Set(dedupedData.map((d) => d.profiled_account).filter(Boolean)),
        ],
        recruiter: [
          ...new Set(dedupedData.map((d) => d.recruiter).filter(Boolean)),
        ],
        applied_position_title: [
          ...new Set(
            dedupedData.map((d) => d.applied_position_title).filter(Boolean),
          ),
        ],
      });
    } catch (err) {
  ...
  // ✅ useEffect only triggers fetch on mount
  useEffect(() => {
    fetchData();
  }, []);
