...
function RecruitmentTracker() {
  ...
  const dedupeByApplicationId = (applicants) => {
    const map = new Map();

    for (const applicant of applicants) {
      const existing = map.get(applicant.applicationid);
      if (!existing || applicant.ID > existing.ID) {
        map.set(applicant.applicationid, applicant);
      }
    }
  ...
  const fetchData = async () => {
    try {
      ...
      const normalized = dedupedData.map(normalizeRow);

      setTrackers(normalized);
      setFilteredTrackers(normalized);
      setError(null); // Clear previous error

      console.log(`✅ Recruitment Tracker: ${normalized.length} rows loaded`);
    } catch (error) {
  ...
  const handleUpdateLifecycle = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // 🔥 Merge updated values into selected
      ...
      <div className="flex-1 flex flex-col">
        ...
        focus:ring-2 focus:ring-blue-300 outline-none text-left"
              ...
              <>
                        ...
                        return categories[selectedTab]
                          ...
                          .map((field) => (
                            <Info
                              ...
                              value={renderFieldValue(field, preview?.[field])}
                            />
                          ));
                      })()}
                    </div>
                ...
                <div className="p-6 mb-10 border-t border-slate-200">
                  <div className="flex gap-4">
                    <button
                      onClick={() => {
                        setFormData(normalizeFormData(selected));
          ...
          <UpdateCandidateLifecycleModal
            ...
            onClose={() => setIsUpdateCycleModalOpen(false)}
            selected={selected}
            formData={formData}
