...
const BUCKET_NAME = "cmxphrecruitmentbucket"; // Replace with your S3 bucket name
...
const authOtpRoute = require("./routes/authOtpRoute");
...
app.get("/api/applicantList", async (req, res) => {
  ...
  console.log("🚨 applicantList endpoint HIT. Fetching all records...");
  ...
  try {
    ...
    const updatedRows = rows.map((row) => ({
      ...row,
      resumeUrl: null, // optional cleanup
    }));

    return res.json({ source: "mysql", data: updatedRows });
  } catch (error) {
...
app.get("/api/taskList", async (req, res) => {
  ...
  const query = `
    ...
    FROM 1000_cmx_appdata_client_database.db_cmx_client_roster
    WHERE ACCOUNT = ? AND LOB = ? AND SITE = 'DR'
      AND ID IN (
        SELECT MAX(ID)
        FROM 1000_cmx_appdata_client_database.db_cmx_client_roster
        WHERE SITE= 'DR'
...
app.get("/api/accountCode", async (req, res) => {
  ...
  const query = `
    SELECT ACCOUNTCODE
    FROM 1000_cmx_appdata_client_database.db_cmx_client_roster
    WHERE ACCOUNT = ? AND LOB = ? AND TASK = ? AND SITE= 'DR'
      AND ID IN (
        SELECT MAX(ID)
        FROM 1000_cmx_appdata_client_database.db_cmx_client_roster
        WHERE SITE = 'DR'
...
app.post("/addApplicants", upload.single("resume"), async (req, res) => {
  try {
    ...
    const roleAbbreviation = data.roleApplied
      ? data.roleApplied.slice(0, 2).toUpperCase()
      : "XX";
